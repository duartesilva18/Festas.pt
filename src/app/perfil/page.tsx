...
export default async function PaginaPerfil() {
  ...
  const [{ data: favoritos }, { data: pedidos }, { data: edicoes }] = await Promise.all([
    ...
    supabase.from("edicoes").select("id,ano,data_inicio,data_fim,dias_semana,estado,cartaz_url,created_at,festa:festas(id,slug,nome,concelho,concelho_slug,distrito,tipo_recorrencia)").eq("criado_por", user.id).order("data_inicio", { ascending: false }),
  ]);
