...
type EventoCriado = {
  ...
  cartaz_url: string | null;
  festa: { id: string; slug: string; nome: string; concelho: string; concelho_slug: string; distrito: string }[] | null;
};
...
const CRITICAS_DEMONSTRACAO: CriticaRecebida[] = [
  { id: "demo-1", festa_id: "demo-1", evento_nome: "Feiras Novas", autor_nome: "Marta Rodrigues", nota: 5, texto: "Uma festa muito bem organizada, com excelente ambiente e um programa para todas as idades.", created_at: "2026-07-18T12:00:00Z" },
  { id: "demo-2", festa_id: "demo-2", evento_nome: "Romaria de Nossa Senhora d'Agonia", autor_nome: "Tiago Almeida", nota: 4, texto: "Boa música e animação. A informação sobre os acessos podia estar um pouco mais visível.", created_at: "2026-07-16T12:00:00Z" },
  { id: "demo-3", festa_id: "demo-3", evento_nome: "Festas de Nossa Senhora da Boa Viagem", autor_nome: "Carolina Lopes", nota: 5, texto: "Adorei a experiência. Espaço cuidado, boa comida e uma equipa sempre disponível.", created_at: "2026-07-13T12:00:00Z" },
];
...
export default async function PaginaPerfil() {
  ...
  const { data: perfil } = await supabase.from("perfis").select("nome,email,avatar_url,papel").eq("id", user.id).single();
  ...
  const [{ data: favoritos }, { data: pedidos }, { data: edicoes }] = await Promise.all([
    supabase.from("favoritos").select("festa_id").eq("user_id", user.id),
    ...
    supabase.from("edicoes").select("id,ano,data_inicio,data_fim,estado,cartaz_url,created_at,festa:festas(id,slug,nome,concelho,concelho_slug,distrito)").eq("criado_por", user.id).order("data_inicio", { ascending: false }),
  ]);

  const ids = (favoritos ?? []).map((f) => f.festa_id as string);
  let festas: FestaGuardada[] = [];
  if (ids.length) {
    const { data } = await supabase.from("festas_mapa").select("id,slug,nome,concelho,concelho_slug,distrito,data_inicio,data_fim,cartaz_url").in("id", ids).order("data_inicio");
  ...
  const mostrarDemonstracaoCriticas = criticasRecebidas.length === 0;
  const criticasVisiveis = mostrarDemonstracaoCriticas ? CRITICAS_DEMONSTRACAO : criticasRecebidas;
  const criticasPerfil = criticasVisiveis.map((critica) => ({ id: critica.id, autor: critica.autor_nome ?? "Participante", evento: critica.evento_nome ?? nomesFestasCriadas.get(critica.festa_id) ?? "Evento", nota: critica.nota, texto: critica.texto, criadaEm: critica.created_at }));
  ...
  const proximoEvento = eventosCriados.filter((evento) => evento.estado !== "cancelada" && evento.data_inicio >= hoje).sort((a, b) => a.data_inicio.localeCompare(b.data_inicio))[0] ?? null;
