...
const inputClasseInvalido = "mt-1.5 w-full rounded-lg border border-[#c43d4b]/60 bg-[#c43d4b]/[0.03] px-3 py-2.5 text-sm text-[#102745] outline-none transition placeholder:text-[#1A2E4F]/35 focus:border-[#c43d4b] focus:ring-2 focus:ring-[#c43d4b]/15";
const labelClasse = "block text-xs font-bold text-[#102745]";
function classeCampo(invalido: boolean) { return invalido ? inputClasseInvalido : inputClasse; }
...
export default function CriadorEvento({
  ...
  // Só marca campos a vermelho depois de uma tentativa falhada de avançar —
  // nunca ao abrir um passo em branco, que pareceria um erro sem motivo.
          ...
          {passo === 0 && <div>
            ...
            <div className="space-y-5">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <label className={labelClasse}>{dados.recorrencia === "fins_de_semana" ? "Começa a partir de" : "Começa em"} <span className="text-[#EC2456]">*</span><input type="date" value={dados.dataInicio} onChange={(e) => alterar("dataInicio", e.target.value)} className={classeCampo(mostrarErros && !dados.dataInicio)} /></label>
                <label className={labelClasse}>{dados.recorrencia === "fins_de_semana" ? "Repetir até" : "Termina em"} {dados.recorrencia === "fins_de_semana" ? <span className="text-[#EC2456]">*</span> : <span className="font-normal text-[#1A2E4F]/40">(opcional)</span>}<input type="date" min={dados.dataInicio || undefined} value={dados.dataFim} onChange={(e) => alterar("dataFim", e.target.value)} className={classeCampo(mostrarErros && ((Boolean(dados.dataFim) && Boolean(dados.dataInicio) && dados.dataFim < dados.dataInicio) || (dados.recorrencia === "fins_de_semana" && !dados.dataFim)))} /></label>
              </div>
              {dados.recorrencia === "fins_de_semana" && <div className={`rounded-lg border p-4 ${mostrarErros && dados.diasSemana.length === 0 ? "border-[#c43d4b]/60 bg-[#c43d4b]/[0.03]" : "border-[#1A2E4F]/10 bg-[#1A2E4F]/[0.025]"}`}>
