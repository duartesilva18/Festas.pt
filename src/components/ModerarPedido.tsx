...
export default function ModerarPedido({ pedidoId }: { pedidoId: string }) {
  ...
  const moderar = async (acao: "aprovar" | "rejeitar") => {
    ...
    try {
      const resposta = await fetch("/api/admin/pedidos", {
        ...
        body: JSON.stringify({ id: pedidoId, acao, nota }),
      });
      if (!resposta.ok) {
  ...
  return (
    <div>
      <input
        ...
        maxLength={500}
        placeholder="Nota para o utilizador (opcional, mostrada se rejeitares)"
